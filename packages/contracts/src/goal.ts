...
const GoalRunBindingV1CompatSchema = z.object({
  ...
  policyRevision: z.union([z.number().int().nonnegative(), z.string().min(1).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u)]),
  capabilityProfileRevision: z.union([z.number().int().nonnegative(), z.string().min(1).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u)]),
  approvalPolicyRevision: z.union([z.number().int().nonnegative(), z.string().min(1).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u)]),
  sandboxSnapshotRevision: z.union([z.number().int().nonnegative(), z.string().min(1).max(128).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u)]),
