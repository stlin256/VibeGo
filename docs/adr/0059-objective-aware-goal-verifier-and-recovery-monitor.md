...
1. Add an optional, versioned `GoalVerificationPlanV1` to a Todo. The plan is
   structured acceptance criteria: required/forbidden event types, a bounded
   minimum output byte threshold, and fixed semantic assertions over
   server-derived facts (`run.exitReason`, model finish reason, tool success/
   bytes/truncation, approval decisions and turn counters). It contains no
   prompt, transcript, command, path, environment or secret.
...
3. Register a daemon-owned deterministic objective criteria verifier for the
   ...
   output and every semantic assertion satisfy the plan and the objective
   digest is correct. Missing plans/assertions, malformed snapshots or
   contradictory evidence return `inconclusive`. This is structured semantic
   validation over an objective-owned acceptance contract, not an LLM
   self-report and not a second execution authority.
4. Add `GoalRecoveryMonitor` in the daemon application layer. Its serialized
   ...
   observed but not launched. Active runs are not launched twice, retry
   attempts are capped, old reservations are released before a fresh attempt,
   and `retry_created` is recorded idempotently. The monitor never retries a
   successfully completed run whose semantic validation is inconclusive, never
   replays an old tool call, owns a queue, or creates a second scheduler.
...
- Assertions remain bounded and testable without exposing raw model/tool
  output; richer provider/LLM review can be added later behind a new, explicit
  port without weakening this deterministic gate.
