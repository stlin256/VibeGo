...
1. 英文 README、中文 README、docs 索引和用户/维护者入口互相链接且状态一致；
2. 新用户能从 clean checkout 按 Quickstart 在声明的平台完成安装、启动、pairing、配置和
   第一条对话；失败时能找到安全、可操作的排查路径；
3. 默认能力、权限 profile、审批、Goal、真实 LLM、远程访问、证书、安装/升级/回滚和
   未实现项均有准确、无夸大的说明；
4. 文档不要求普通用户手动编辑危险配置文件，不包含任何真实 secret、绝对路径、原始命令
   或完整 transcript；
5. 品牌横幅、架构图、术语、英文优先和中文同步符合本规格；截图若存在则是脱敏真实用户
   界面，不是初始化配置或巨大 Logo mockup；
6. 文档质量门禁、命令验证、链接/fence/diff/privacy 检查和至少三种读者审阅均有证据；
7. 文档更新在对应代码/测试变更之前完成，并以独立 Git 提交记录；发布说明引用 Spec 60
