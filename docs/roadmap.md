...
**状态：Accepted（文档阶段已完成；阶段 1–2、认证门禁、Web/PWA、LAN TLS、Skill/MCP manifest 与 Sandbox runtime plan MVP 已落地）**
...
| 5 | 沙箱 | host-restricted adapter、Docker/Podman 命令计划、资源限制 | 命令计划与 resolver 隔离强度测试通过（已完成）；真实 runtime adapter 后置 |
...
当前已完成 `contracts → testkit → in-memory event storage → scheduler → SQLite EventStore → daemon /health → fake-model loop → run API/SSE → ContextManager/provider → agent 接入 → ToolRegistry/ApprovalPolicy → sandbox/execution 安全边界 → tool adapter → auth/transport 门禁 → Web/PWA MVP → LAN TLS MVP → Skill/MCP manifest/allowlist MVP → Sandbox runtime 命令计划 MVP`；随后接真实 Docker/Podman adapter、ACME/certificate manager adapter 与 UI 深化。
