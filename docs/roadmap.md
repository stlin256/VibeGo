...
**状态：Accepted（阶段 1–2、认证门禁、Web/PWA、LAN TLS、Skill/MCP manifest、Sandbox runtime 与 guided workspace registry MVP 已落地；Spec 53 Phase 0/1/2/3/4/5/6 与 Spec 57 Phase 57a 已实现，其余 release-hardening 阶段仍为规划）**
...
## Spec 42：shadcn 风格 Web 设计系统与 conversation-first UI（Phase 42a/42b-1 已实现，42b-2/42c/42d 后续）
...
键盘、可访问性和 bundle 验收。当前 Web focused gate 为 78 tests、typecheck
和 production build；观测到 JS gzip 79.87 KiB、CSS gzip 5.82 KiB。
...
仍由 App 注入，不创建第二条 SSE 或事实源；workspace rail、context rail 与
Settings drawer 留在后续 42b-2/42c 切片。
