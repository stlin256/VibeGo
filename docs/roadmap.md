...
**状态：Accepted（文档阶段已完成；阶段 1–2、认证门禁、Web/PWA、LAN TLS 与 Skill/MCP manifest MVP 已落地）**
...
当前已完成 `contracts → testkit → in-memory event storage → scheduler → SQLite EventStore → daemon /health → fake-model loop → run API/SSE → ContextManager/provider → agent 接入 → ToolRegistry/ApprovalPolicy → sandbox/execution 安全边界 → tool adapter → auth/transport 门禁 → Web/PWA MVP → LAN TLS MVP → Skill/MCP manifest/allowlist MVP`；随后接真实 transport adapter、ACME/certificate manager adapter、external sandbox runtime 与 UI 深化。
