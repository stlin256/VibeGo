...
- [Spec 39：TencentDB Agent Memory 可切换融合与自动更新](specs/39-tencentdb-agent-memory-integration.md) 与 [ADR 0008](adr/0008-tencentdb-agent-memory-sidecar-and-live-update.md)：Phase 0–5 已实现 contract、MemoryCore、settings、supervisor、bounded run integration、显式 MemoryProxy adapter 和只读 MemoryKnowledge adapter；Phase 6a 已实现独立 Knowledge settings、认证 probe、bounded new-run context 注入和 Web 状态卡片。Knowledge 工具化、Proxy sidecar 自动构建/切换与运营 history 后置，记忆不可用不阻塞 Web/run。
