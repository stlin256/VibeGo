...
- [Spec 39：TencentDB Agent Memory 可切换融合与自动更新](specs/39-tencentdb-agent-memory-integration.md) 与 [ADR 0008](adr/0008-tencentdb-agent-memory-sidecar-and-live-update.md)：Phase 0 已实现原生 contract/Noop 边界；后续采用独立 sidecar、原生 Adapter、Web 开关和 GitHub 上游候选构建/健康检查/切换/回滚，MemoryCore 为首选，Proxy/Knowledge 后置，记忆不可用不阻塞 Web/run。
