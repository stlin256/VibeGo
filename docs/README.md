...
- [Spec 46：Automated verification workflow](specs/46-automated-verification-workflow.md) 与 [ADR 0015](adr/0015-automated-verification-workflow.md)：固定 `pnpm verify` 的 typecheck → test → diff-check → Git diff-check 顺序，不安装依赖、不访问模型、不输出环境 secret。
