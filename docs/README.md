...
- [Spec 39：TencentDB Agent Memory 可切换融合与自动更新](specs/39-tencentdb-agent-memory-integration.md) 与 [ADR 0008](adr/0008-tencentdb-agent-memory-sidecar-and-live-update.md)：Phase 0–4 已实现 contract、MemoryCore、settings、supervisor 和 bounded run integration；Phase 5 已实现显式 MemoryProxy adapter，以及只读 MemoryKnowledge descriptor/工具白名单、bounded/cancellable retrieval 和 privacy boundary。Knowledge 应用服务注入与 Proxy sidecar 自动构建/切换后置，记忆不可用不阻塞 Web/run。
