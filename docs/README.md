...
- [Spec 61：DeepSeek 一等 Provider、思考模式与低打扰 Agent Loop](specs/61-deepseek-first-class-provider-integration.md)：吸收 MinimumAgentLoop 的两层循环、多工具调用、thinking、advisory reviewer 和 provider-owned search 思路；61-0 clean-room 证据见 [ADR 0045](adr/0045-deepseek-provider-clean-room-boundary.md)，61-1 contract 与 61-2 三协议 adapter 已完成，61-3 application snapshot integration 实施中，仍保留 VibeGo 的 ModelProvider、Approval、Sandbox、Goal、Scheduler 和事件事实源。
