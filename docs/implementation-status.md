...
49. `docs/specs/54-model-provider-onboarding.md` 已实现 Phase 0 strict onboarding contracts：`ModelProviderDescriptor`、`ModelEndpointProfile`、`ModelCredentialRef`、capability/probe/setup-session 均有版本、bounded、privacy/path 校验；unknown capability 保留 unknown 语义，credential 仅为 opaque ref。当前不改 Spec 28 的 provider/runtime、AgentLoop、RunManager 或 secret 边界。
