...
50. `docs/specs/56-i18n-accessibility-device-matrix.md` 已冻结 Phase 56a 边界并新增 `docs/adr/0024-web-locale-and-accessibility-shell.md`：先实现 Web-only `en-US`/`zh-CN` locale preference、英文 fallback、根节点 `lang`、核心 shell 的 bounded accessibility 语义和 ratio-first focused gates；完整 catalog、真实设备和屏幕阅读器人工 evidence 尚未声称完成。
