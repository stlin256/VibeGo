...
54. Spec 55 Phase 55b contract boundary 已冻结：复用现有 AuthGate 暴露只读 `GET /api/v1/deployment/readiness`，Web 仅显示 bounded mode/status/reason/next-step；缺失 projection fail-soft，不改变 pairing、interactive run、AgentLoop、run_events 或 transport listener，也不接受任何 deployment mutation。
