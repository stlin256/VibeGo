...
**状态：Accepted（Agent Memory Phase 6b 与 Goal Control Phase 2A 已实现；Web/PWA、LAN TLS、Skill/MCP manifest、Sandbox runtime、ToolRuntime、approval continuation 与 Goal 只读投影切片已通过；Spec 53 Phase 0/1/2/3 与 Spec 57 Phase 57a 已实现，其余 release-hardening 阶段仍为规划）**
...
48. `docs/specs/53-host-install-upgrade-backup-recovery.md` 已实现 Phase 0/1/2/3：`host-manifest/v1`、`host_update_state_v1`、`backup-manifest/v1`/restore/recovery/diagnostic strict contracts 与 `SqliteBackupSnapshotAdapter` 均有 bounded/privacy/path/integrity 校验；未知字段、credential/query token、绝对路径、无效时间、跳过验证、无 previous 回滚、credential/workspace-file import、越权 safe-mode operation、损坏数据库、schema mismatch、超限输出和覆盖既有 snapshot 均 fail-closed。contracts focused suite 63 tests、storage focused suite 35 tests（含新增 snapshot fixture 4 tests）、storage typecheck/build 通过。该切片不下载、验证、安装、迁移、restore、打包、切换或回滚 artifact，也不改变 daemon、workspace、run/Goal 事实源。
