...
- `pnpm test`：通过，244 个测试全部通过（contracts 12、goal-control 11、storage 17、scheduler 5、testkit 2、agent 20、context 5、model-openai 5、tools 4、policy 7、sandbox 6、execution 7、sandbox-runtime 9、tool-adapters 15、workspaces 7、auth 5、certificates 5、skill-mcp 10、daemon 56、web 36；Vitest 按 package 输出）；Spec 36 覆盖 settings store、workspace restore/rollback 和 daemon adapter；Spec 37/38 覆盖 ratio、conversation-first 与 New task focus contract；Spec 39 覆盖 Phase 0 contract privacy/bounds、Noop zero-side-effect 和 Phase 1 MemoryCore adapter degraded/bounded/identity/write-back 行为；
...
and 206 passing tests. The current verification is 20 workspace packages and 244
passing tests (Web 36, daemon 56, storage 17, contracts 12, workspaces 7).
Specs 36–39 add
...
contract/Noop boundary and Phase 1 MemoryCore adapter boundary, with focused
client/component and regression tests; screenshot files are not part of the
repository. It keeps SQLite `goal_events` and the daemon
