...
49. `docs/specs/54-model-provider-onboarding.md` 已实现 Phase 0/1：`ModelProviderDescriptor`、`ModelEndpointProfile`、`ModelCredentialRef`、capability/probe/setup-session contracts，以及显式 OpenAI-compatible `/models` probe 均有版本、bounded、privacy/path 校验；unknown capability 保留 unknown 语义，credential 仅为 opaque ref。probe 不发送 prompt、不创建 run/event、不改变 provider 或 in-flight snapshot；当前仍不改 Spec 28 的默认 runtime/secret 边界。
