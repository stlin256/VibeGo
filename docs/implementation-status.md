...
47a. `docs/specs/44-provider-usage-management-and-upstream-reuse.md`、`docs/adr/0013-upstream-research-and-provider-management-boundary.md` 和 `docs/prompts/44-provider-usage-management-implementation.md` 已补齐 Provider/Token/费用管理的上游研究门禁、许可证/NOTICE/provenance 规则、clean-room 复用边界和分阶段 Agent 指引。当前只完成文档冻结；CC Switch、AxonHub 等上游源码的 pinned commit 尚未写入 `docs/research/upstream-provider-usage.md`，因此 44-R0 仍是实现前置门禁，不能把未核验的仓库当作复制来源。
