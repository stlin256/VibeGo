...
48. `docs/specs/53-host-install-upgrade-backup-recovery.md` 已实现 Phase 0 `host-manifest/v1` strict contract：平台目标、runtime/web revision、数据库兼容范围、SHA-256 digest、签名/attestation 引用和创建时间均有 bounded/privacy/path 校验；未知字段、credential/query token、绝对路径和无效时间 fail-closed。该切片不下载、验证、安装、迁移、切换或回滚 artifact，也不改变 daemon、workspace、run/Goal 事实源。
