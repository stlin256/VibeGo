...
**状态：Accepted（Phase 1/2 实施基线，tool adapter 纵切已通过）**
...
13. 每个包都有单元测试和 typecheck；根目录 `build` 会按 contracts → storage → scheduler → testkit → context → agent → model-openai → tools → policy → sandbox → execution → tool-adapters → daemon 顺序构建，避免 workspace package export 在 clean checkout 下缺少 `dist` 类型。

## 验证结果（2026-08-03）

- `pnpm typecheck`：通过（13 个 workspace package）；
- `pnpm test`：通过，75 个测试全部通过（contracts 3、storage 6、scheduler 5、testkit 2、agent 9、context 5、model-openai 4、tools 4、policy 7、sandbox 6、execution 7、tool-adapters 9、daemon 8；Vitest 按 package 输出）；
- `pnpm diff:check`：通过；
...
- ToolRegistry/ApprovalPolicy 目前只做元数据和判定，不执行任何真实 tool；sandbox provider 已实现 verifier/resolver，真实 Docker/Podman/VM 执行器后置；tool-adapters 只在显式注入 runner/filesystem 时执行，daemon 尚未 wiring。
...
- 文档中的实现状态、限制和命令与代码一致；
- 完成后单独 Git 提交，再进入 external sandbox runtime、认证门禁与 Web UI。
