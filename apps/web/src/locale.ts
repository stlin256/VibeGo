...
const EN_US: Catalog = {
  ...
  'settings.git.eyebrow': 'GIT TOOLS',
  'settings.git.title': 'Git tools',
  'settings.git.description': 'Status, diff, log, plus approval-gated writes: add, commit, branch. Push, reset and restore are destructive and always ask for explicit approval.',
  'settings.git.ariaLabel': 'Git tool setup',
  'settings.git.unavailableNote': 'Git settings are unavailable until the daemon exposes the authenticated adapter.',
  'settings.git.enableLabel': 'Enable Git tools',
  'settings.git.note': 'Workspace: {workspace}. Exposes status, diff, log, add, commit, branch, push, reset and restore. Write operations require approval; destructive operations (push, reset, restore) require explicit approval for every call.',
  'settings.git.unpairedNote': 'Pair with the daemon to configure Git tools.',
...
const ZH_CN: Catalog = {
  ...
  'settings.git.eyebrow': 'GIT 工具',
  'settings.git.title': 'Git 工具',
  'settings.git.description': 'status、diff、log 加上需审批的写入操作：add、commit、branch。push、reset、restore 为破坏性操作，每次都会要求显式审批。',
  'settings.git.ariaLabel': 'Git 工具设置',
  'settings.git.unavailableNote': '在 daemon 暴露经认证的适配器之前，Git 设置不可用。',
  'settings.git.enableLabel': '启用 Git 工具',
  'settings.git.note': '工作区：{workspace}。启用 status、diff、log、add、commit、branch、push、reset、restore。写入操作需审批；破坏性操作（push、reset、restore）每次调用都需显式审批。',
  'settings.git.unpairedNote': '与 daemon 配对后即可配置 Git 工具。',
