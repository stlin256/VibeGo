...
const EN_US: Catalog = {
  ...
  'settings.capability.advancedLocalDescription': 'Opt-in host-restricted shell; explicit acknowledgement is required.',
  ...
  'settings.sandbox.description': 'Docker/Podman shell is opt-in and has no host fallback.',
  ...
  'settings.sandbox.note': 'Docker/Podman shell is off by default. Probe the runtime, then enable it explicitly; no host shell fallback exists.',
...
const ZH_CN: Catalog = {
  ...
  'settings.capability.advancedLocalDescription': '显式启用的主机受限 shell；需要明确确认。',
  ...
  'settings.sandbox.description': 'Docker/Podman shell 为显式启用，且没有主机回退。',
  'settings.sandbox.ariaLabel': '外部沙箱设置',
  'settings.sandbox.unavailableNote': '在经认证的适配器就绪之前，外部沙箱设置不可用。',
  'settings.sandbox.note': 'Docker/Podman shell 默认关闭。先探测运行时，再显式启用；不存在主机 shell 回退。',
