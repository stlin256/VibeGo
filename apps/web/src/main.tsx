...
function RuntimeApp(): JSX.Element {
  ...
  const createWorkspace = async (name: string): Promise<string | undefined> => {
    try {
      const next = await client.createWorkspace(name);
      setWorkspaces(next);
      setWorkspacesUnavailable(false);
      setError(undefined);
      const slug = name.trim().toLowerCase().replace(/[^a-z0-9_-]+/gu, '-').replace(/^-+|-+$/gu, '');
      return next.workspaces.find((workspace) => workspace.id === slug)?.id;
    } catch (reason) { setError(safeError(reason, t)); throw reason; }
  };
