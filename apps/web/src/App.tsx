...
export function App({ health, run, events = [], error, onPair, onCreateRun, onCancel, onApprove }: AppProps): JSX.Element {
      ...
      <section className="content-grid">
        ...
        <section className="main-column">
          ...
          {connected ? <>
            ...
            {run ? <RunConsole run={run} events={events} onCancel={onCancel} onApprove={onApprove} /> : <section className="panel empty-state"><span className="empty-icon">⌁</span><h2>还没有活动 run</h2><p className="muted">提交一个任务，实时查看 agent 的计划、输出和审批。</p></section>}
