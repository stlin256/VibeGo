        ...
        <aside className="sidebar" aria-label={t('accessibility.sidebarLabel')}>
          ...
          {!connected && <>
            <section className="panel connection-panel">
              ...
              {health ? <dl className="summary-list"><div><dt>transport</dt><dd>{health.transport.kind}</dd></div><div><dt>TLS</dt><dd>{health.transport.tlsRequired ? 'required' : 'off'}</dd></div><div><dt>sandbox</dt><dd>{health.sandbox.availableModes.join(' · ')}</dd></div></dl> : <p className="muted">{t('connection.readingDaemon')}</p>}
            </section>
            <section className="panel pairing-panel"><div className="eyebrow">{t('connection.pairingEyebrow')}</div><h2>{t('connection.pairingTitle')}</h2>{onPairSeamless && <><Button className="pairing-auto-button" onClick={onPairSeamless}>{t('connection.pairingAuto')}</Button><p className="muted">{t('connection.pairingAutoHint')}</p><div className="pairing-divider"><span>{t('connection.pairingManualDivider')}</span></div></>}<p className="muted">{t('connection.pairingDescription')}</p><form onSubmit={submitPairing}><label htmlFor="pairing-code">{t('connection.pairingCodeLabel')}</label><input id="pairing-code" value={pairingCode} onChange={(event) => setPairingCode(event.target.value)} autoComplete="off" inputMode="text" /><button type="submit">{t('connection.pairingAction')}</button></form></section>
